...
TestHelper.test("EventBusClient:send_event - should use retry manager for async sending", function()
    ...
    mock_retry_manager.execute_with_retry =
        TestHelper.mock_function("retry_manager.execute_with_retry")
...
TestHelper.test(
    "EventBusClient:send_event - should buffer events when circuit breaker is open",
    function()
        EventBusClient.connection_tested = true
        ...
        TestHelper.reset_mocks()
        mock_retry_manager.can_attempt =
            TestHelper.mock_function("retry_manager.can_attempt", false)
...
TestHelper.test(
    "EventBusClient:buffer_event - should drop oldest events when buffer is full",
    function()
        EventBusClient.local_buffer = {}
        EventBusClient.max_buffer_size = 3
...
TestHelper.test(
    "EventBusClient:flush_buffer - should send buffered events when circuit breaker allows",
    function()
        EventBusClient.local_buffer = {
            { type = "BUFFERED_1" },
            { type = "BUFFERED_2" },
