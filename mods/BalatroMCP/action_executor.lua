...
function ActionExecutor:register_handlers()
    ...
    self.action_handlers["go_to_shop"] = function()
        self:go_to_shop()
    end
end
        ...
        if G.FUNCS then
            ...
            for k, v in pairs(G.FUNCS) do
                if
                    ...
                    and (
                        string.find(k, "shop")
                        or string.find(k, "continue")
                        or string.find(k, "cash")
                    )
                then
            ...
            self.logger:debug(
                "Available shop/continue functions",
                { funcs = table.concat(available_funcs, ", ") }
            )
        end
