...
class VersionManager:
    ...
    def register_migration(self, from_version: SchemaVersion, 
    ...
    def check_compatibility(self, client_version: SchemaVersion) -> Tuple[bool, Optional[str]]:
        ...
        if client_version < self.MIN_SUPPORTED_VERSION:
            return False, f"Version {client_version} is below minimum supported version {self.MIN_SUPPORTED_VERSION}"
            
        # Check major version compatibility
        if client_version.major != self.CURRENT_VERSION.major:
            return False, f"Major version mismatch: client {client_version.major}, server {self.CURRENT_VERSION.major}"
            
        # Minor version differences are OK (backward compatible)
        if client_version.minor > self.CURRENT_VERSION.minor:
            return False, f"Client version {client_version} is newer than server version {self.CURRENT_VERSION}"
            
        return True, None
        
    def negotiate_version(self, client_versions: List[SchemaVersion]) -> Optional[SchemaVersion]:
    ...
    def get_required_migrations(self, from_version: SchemaVersion, 
                               to_version: SchemaVersion) -> List[callable]:
    ...
    def apply_migrations(self, data: Dict, from_version: SchemaVersion,
                        to_version: SchemaVersion) -> Dict:
    ...
    def check_feature_availability(self, feature: str, 
                                  client_version: SchemaVersion) -> bool:
    ...
    def get_version_info(self) -> Dict:
        ...
        return {
            ...
            "features": {
                feature: str(version) 
                for feature, version in self.feature_flags.items()
            },
            "migration_paths": [
                {"from": from_v, "to": to_v}
