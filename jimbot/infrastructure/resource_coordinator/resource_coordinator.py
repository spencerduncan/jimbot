...
class GPUAllocator:
    ...
    @asynccontextmanager
    async def allocate(
        self, component_id: str, duration_seconds: Optional[float] = None
    ):
        """
        Allocate GPU to a component.
        ...
        while True:
            if not self.current_user:
                ...
                if next_component == component_id:
                    ...
                    await self.allocation_queue.put(
                        (priority, time.time(), next_component)
                    )
    ...
    def _get_priority(self, component_id: str) -> int:
        ...
        priority_map = {"training": 1, "inference": 2, "evaluation": 3}
...
class RedisCoordinator:
    ...
    async def initialize(self):
        ...
        logger.info(
            f"Redis coordinator initialized with {self.max_connections} max connections"
        )
    ...
    async def get_client(self, namespace: str):
        ...
        if namespace not in self._clients:
            ...
            self._clients[namespace] = (
                f"RedisClient(prefix={self.namespaces[namespace]})"
            )
