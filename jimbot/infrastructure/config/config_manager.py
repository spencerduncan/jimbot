...
class ConfigManager:
    ...
    async def reload(self):
        ...
        env_config = self._load_file(self.config_dir / "environments" / f"{self.environment}.yaml")
        if env_config:
            ...
            if '*' in pattern and self._matches_pattern(key, pattern):
                for handler in handlers:
                    try:
                        ...
                        logger.error(f"Error in pattern watcher {pattern}: {e}", exc_info=True)
