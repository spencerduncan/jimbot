...
class StrategyCache:
    ...
    async def get(self, game_state: Any) -> Optional[Any]:
        ...
        async with self.lock:
            ...
            if similar_entry := self._find_similar(state_vector):
                ...
                logger.debug(f"Cache hit (similarity): confidence={similar_entry.strategy.confidence}")
            ...
            if pattern_entry := self.pattern_cache.get(pattern_key):
                if not pattern_entry.is_expired(self.ttl_hours * 2):  # Patterns last longer
    ...
    async def put(self, game_state: Any, strategy: Any):
        ...
        async with self.lock:
            ...
            if len(self.similarity_cache) > self.max_size // 2:
                ...
                self.similarity_cache.sort(key=lambda e: e.success_rate * e.hits, reverse=True)
    ...
    async def clear_expired(self):
        ...
        async with self.lock:
            ...
            self.similarity_cache = [
                e for e in self.similarity_cache 
                if not e.is_expired(self.ttl_hours)
            ]
            
            # Clear pattern cache (longer TTL)
            expired_patterns = [
                k for k, v in self.pattern_cache.items()
    ...
    def _hash_game_state(self, game_state: Any) -> str:
        ...
        state_dict = asdict(game_state) if hasattr(game_state, '__dict__') else game_state
    ...
    def _evict_lru(self):
        ...
        for key, entry in self.exact_cache.items():
            ...
            performance_penalty = (1 - entry.success_rate) * 3600  # 1 hour penalty per failure
            score = recency_score + performance_penalty - (entry.hits * 600)  # 10 min bonus per hit
