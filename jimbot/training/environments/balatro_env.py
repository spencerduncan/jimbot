...
class BalatroEnv(gym.Env):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.observation_space = spaces.Dict({
            ...
            "game_state": spaces.Box(low=-np.inf, high=np.inf, shape=(50,), dtype=np.float32),
            
            # Knowledge graph embedding from Memgraph
            "memgraph_embedding": spaces.Box(low=-1, high=1, shape=(128,), dtype=np.float32),
            
            # Valid actions mask
            "action_mask": spaces.Box(low=0, high=1, shape=(1000,), dtype=np.float32)
        })
    ...
    def step(self, action: int) -> Tuple[Dict[str, np.ndarray], float, bool, Dict[str, Any]]:
        ...
        # Check if done
            ...
            game_response.get("game_over", False) or 
