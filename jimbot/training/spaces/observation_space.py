...
class DynamicObservationSpace:
    ...
    # Feature dimensions
    CARD_FEATURES = 8  # suit, rank, enhancement, seal, edition, location, selected, playable
    JOKER_FEATURES = 10  # id, level, edition, sell_value, position, + 5 extra data slots
    GLOBAL_FEATURES = 20  # money, ante, round, blind_type, chips_required, hands_left, etc.
    ...
    def _encode_cards(self, cards: List[Card], max_length: int, 
                     location: str = 'unknown',
                     selected_indices: Optional[set] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _encode_jokers(self, jokers: List[Joker]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i, joker in enumerate(jokers[:self.MAX_JOKERS]):
            ...
            # Basic features
            features[i, 0] = joker.joker_id / 150.0  # Normalize assuming ~150 joker types
            ...
            features[i, 4] = joker.position / (self.MAX_JOKERS - 1) if self.MAX_JOKERS > 1 else 0
    ...
    def _dict_to_card(self, card_dict: Dict[str, Any]) -> Card:
        ...
        return Card(
            ...
            enhancement=CardEnhancement(card_dict.get('enhancement', CardEnhancement.NONE)),
