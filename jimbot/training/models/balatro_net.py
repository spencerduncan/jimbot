...
class BalatroNet(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        self.policy_head = SlimFC(
            hidden_size, num_outputs, initializer=normc_initializer(0.01)
        )

        # Value head
        self.value_head = SlimFC(hidden_size, 1, initializer=normc_initializer(1.0))
    ...
    def forward(
        ...
        combined_features = torch.cat(
            [card_features, joker_features, state_features, memgraph_embedding], dim=1
        )
    ...
    def metrics(self) -> Dict[str, TensorType]:
        ...
        return {
            "feature_norm": (
                torch.mean(torch.norm(self._features, dim=1))
...
class BalatroLSTMNet(BalatroNet):
    ...
    def __init__(
        ...
        hidden_size = model_config.get("custom_model_config", {}).get(
            "hidden_size", 512
        )
        self.lstm = nn.LSTM(
