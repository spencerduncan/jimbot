...
class MockResourceCoordinator(resource_coordinator_pb2_grpc.ResourceCoordinatorServicer):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # Default capacities
        self.capacities = {
            resource_coordinator_pb2.RESOURCE_TYPE_GPU: self.config.get('gpu_capacity', 24000),  # 24GB
            resource_coordinator_pb2.RESOURCE_TYPE_CLAUDE_API: self.config.get('claude_capacity', 100),  # 100 req/hr
            resource_coordinator_pb2.RESOURCE_TYPE_MEMORY: self.config.get('memory_capacity', 32768),  # 32GB
            resource_coordinator_pb2.RESOURCE_TYPE_CPU: self.config.get('cpu_capacity', 16),  # 16 cores
        }
    ...
    def RequestResource(self, request, context):
        ...
        self.logger.info(f"Resource request from {request.component}: "
        ...
        if self.response_mode == 'always_grant':
            ...
            if self.usage[request.resource] + request.quantity <= self.capacities[request.resource]:
                response.status = resource_coordinator_pb2.REQUEST_STATUS_GRANTED
            else:
                response.status = resource_coordinator_pb2.REQUEST_STATUS_DENIED
        ...
        if response.status == resource_coordinator_pb2.REQUEST_STATUS_GRANTED:
            ...
            response.message = "Request queued, will be processed when resources are available"
        
        # Record response
        request_record['status'] = resource_coordinator_pb2.RequestStatus.Name(response.status)
    ...
    def ReleaseResource(self, request, context):
        ...
        self.logger.info(f"Release request from {request.component} for {request.request_id}")
...
def serve(port: int = 50051, config: Optional[Dict] = None):
    ...
    resource_coordinator_pb2_grpc.add_ResourceCoordinatorServicer_to_server(coordinator, server)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', choices=['always_grant', 'simulate_contention', 'respect_limits'],
                       default='always_grant', help='Response mode')
    parser.add_argument('--deny-probability', type=float, default=0.1,
                       help='Probability of denying requests in simulate_contention mode')
    parser.add_argument('--queue-probability', type=float, default=0.2,
