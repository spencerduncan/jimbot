...
class MockEventGenerator:
    ...
    def generate_hand_played(self) -> Dict:
        ...
        hand_types = ["pair", "two_pair", "three_of_a_kind", "straight", 
                      "flush", "full_house", "four_of_a_kind", "straight_flush"]
        
        hand_type = random.choice(hand_types)
        base_scores = {
            "pair": 10, "two_pair": 20, "three_of_a_kind": 30,
            "straight": 30, "flush": 35, "full_house": 40,
            "four_of_a_kind": 60, "straight_flush": 100
        }
...
async def send_mock_events(
    url: str = "ws://localhost:8765/events",
    rate: int = 10,
    duration: int = 60
):
    """
    ...
    async with MCPClient(url) as client:
        ...
        while time.time() - start_time < duration:
            ...
            event_type = random.choices(
                ["hand_played", "shop_entered", "card_purchased"],
                weights=[70, 20, 10]
            )[0]
...
async def main():
    ...
    parser.add_argument("--url", default="ws://localhost:8765/events", 
                        help="MCP server URL")
    parser.add_argument("--test-connection", action="store_true",
                        help="Test connection and exit")
    parser.add_argument("--mock-events", action="store_true",
                        help="Send mock events")
    parser.add_argument("--rate", type=int, default=10,
                        help="Events per second for mock mode")
    parser.add_argument("--duration", type=int, default=60,
                        help="Duration in seconds for mock mode")
