...
class EventAggregator:
    ...
    async def add_event(self, event: Dict) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Event queue full (size={self.event_queue.qsize()}), "
                "dropping event"
            )
            return False
        ...
        while (time.time() - batch_start) < self.batch_window:
            ...
            try:
                ...
                if len(events) >= self.max_batch_size:
                    logger.debug(
                        f"Batch size limit reached ({self.max_batch_size})"
                    )
                    break
    ...
    async def _process_batch(self, events: List[Dict]):
        ...
        try:
            ...
            logger.error(
                f"Error processing batch of {len(events)} events: {e}",
                exc_info=True
            )
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "avg_batch_size": sum(s.event_count for s in recent_stats) / len(recent_stats),
