...
private:
    ...
    int countBits(uint64_t bits) const {
 ...
 */
void calculate_best_hand(mgp_list *args, mgp_graph *graph, 
    ...
    for (size_t i = 0; i < num_cards; ++i) {
        ...
        if (mgp_vertex_get_property(vertex, "enhancement", memory, &prop_val) == MGP_ERROR_NO_ERROR) {
            card.enhancement = mgp_value_get_string(prop_val);
        }
        if (mgp_vertex_get_property(vertex, "base_chips", memory, &prop_val) == MGP_ERROR_NO_ERROR) {
    ...
    if (cards.size() <= 10) {
        ...
        do {
            ...
            double score = result.base_chips + result.base_mult * 50.0 + 
                          result.strength_score * 1000.0;
 ...
 */
void calculate_score_with_jokers(mgp_list *args, mgp_graph *graph,
                               mgp_result *result, mgp_memory *memory) {
...
int mgp_init_module(mgp_module *module, mgp_memory *memory) {
    ...
    {
        mgp_proc *proc = mgp_module_add_read_procedure(module, "calculate_best_hand", 
                                                      calculate_best_hand);
    ...
    {
        mgp_proc *proc = mgp_module_add_read_procedure(module, "calculate_score_with_jokers",
                                                      calculate_score_with_jokers);
