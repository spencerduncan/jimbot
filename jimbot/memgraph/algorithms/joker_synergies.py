...
class JokerSynergyCalculator:
    ...
    async def calculate_pairwise_synergy(
        ...
        async with self.driver.session() as session:
            result = await session.run(query, joker1=joker1, joker2=joker2)
            ...
            confidence = min(
                1.0, record["games_count"] / (self.min_games_for_confidence * 5)
            )
    ...
    async def update_all_synergies(self) -> Dict[str, int]:
        ...
        """
        stats = {"updated": 0, "created": 0, "removed": 0, "errors": 0}
    ...
    async def get_anti_synergies(
        ...
        async with self.driver.session() as session:
            ...
            return [(r["joker1"], r["joker2"], r["strength"]) async for r in result]
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            ...
            synergy = await calculator.calculate_pairwise_synergy(
                "Fibonacci", "Odd Todd"
            )
            if synergy:
