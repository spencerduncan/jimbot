...
class QueryBuilder:
    ...
    def match(self, pattern: str, match_type: MatchType = MatchType.SIMPLE) -> 'QueryBuilder':
        """Add a MATCH clause.
    ...
    def where_in(self, property_path: str, values: List[Any], param_name: str) -> 'QueryBuilder':
    ...
    def order_by(self, expression: str, direction: OrderDirection = OrderDirection.ASC) -> 'QueryBuilder':
        """Add ORDER BY clause.
    ...
    def analyze_deck_compatibility(
        ...
        return (query
            ...
            .with_clause(
                "j",
                "c.suit as suit",
                "SUM(r.optimal_quantity) as needed"
            )
            .where(" AND ".join([
                f"(suit = '{suit}' AND needed <= ${f'deck_{suit.lower()}')})"
