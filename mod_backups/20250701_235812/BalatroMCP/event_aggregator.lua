...
function EventAggregator:aggregate_events(events)
    ...
    if #game_states > 1 then
        ...
        for _, state in ipairs(game_states) do
            ...
            if not latest_by_frame[frame] or 
               state.timestamp > latest_by_frame[frame].timestamp then
...
function EventAggregator:get_stats()
    return {
        ...
        queue_size = #self.event_queue,
        avg_events_per_batch = self.stats.batches_sent > 0 and 
            (self.stats.events_sent / self.stats.batches_sent) or 0
    }
end
