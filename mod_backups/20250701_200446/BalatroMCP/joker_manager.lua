...
function JokerManager:safe_validate_joker(joker, joker_index, operation)
    if not joker then
        if self.crash_diagnostics then
            self.crash_diagnostics:log(
                "ERROR: Joker at index " .. tostring(joker_index) .. " is nil during " .. operation
            )
        end
        return false
    end
    ...
    if not joker.config then
        if self.crash_diagnostics then
            self.crash_diagnostics:log(
                "ERROR: Joker at index "
                    .. tostring(joker_index)
                    .. " has nil config during "
                    .. operation
            )
            self.crash_diagnostics:validate_object_config(
                joker,
                "joker[" .. joker_index .. "]",
                operation
            )
    ...
    if not key then
        if self.crash_diagnostics then
            self.crash_diagnostics:log(
                "ERROR: Joker at index "
                    .. tostring(joker_index)
                    .. " has nil config.center.key during "
                    .. operation
            )
        end
        return nil
...
function JokerManager:setup_post_hand_hook()
    ...
    if self.crash_diagnostics then
        G.FUNCS.evaluate_play = self.crash_diagnostics:create_safe_hook(function(...)
...
function JokerManager:get_blueprint_brainstorm_optimization()
    ...
    if self.crash_diagnostics then
        self.crash_diagnostics:log(
            "OPTIMIZATION: Analyzing " .. #jokers .. " jokers for Blueprint/Brainstorm optimization"
        )
    end
    ...
    for i, joker in ipairs(jokers) do
        ...
        elseif joker_key then
            ...
            if self.crash_diagnostics then
                self.crash_diagnostics:log(
                    "WARNING: Joker at index "
                        .. i
                        .. " has no valid key, skipping from optimization"
                )
            end
        end
    end
    ...
    if self.crash_diagnostics then
        self.crash_diagnostics:log(
            "OPTIMIZATION: Generated order with " .. #optimal_order .. " jokers"
        )
    end
...
function JokerManager:get_joker_info()
    ...
    for i, joker in ipairs(G.jokers.cards) do
        ...
        if self:safe_validate_joker(joker, i, "get_joker_info") then
            ...
            if self.crash_diagnostics then
                self.crash_diagnostics:log(
                    "WARNING: Joker " .. i .. " has corrupted config, using safe defaults"
                )
            end
            info.key = "unknown"
