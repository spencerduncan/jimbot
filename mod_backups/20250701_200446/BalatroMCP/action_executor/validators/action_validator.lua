...
function ActionValidator:initialize()
    ...
    for action_type, validator_list in pairs(self.validators) do
        for _, validator in ipairs(validator_list) do
            if validator.initialize then
                ...
                if not success then
                    print(
                        "BalatroMCP: Warning - Failed to initialize validator for "
                            .. action_type
                            .. ": "
                            .. tostring(error_msg)
                    )
    ...
    print(
        "BalatroMCP: ActionValidator framework initialized with "
            .. self:get_validator_count()
            .. " validators"
    )
end
    ...
    for i, validator in ipairs(validators) do
        ...
        if not success then
            ...
            return ValidationResult.error(
                "Validator error for " .. action_type .. ": " .. tostring(result)
            )
        end
    ...
    for action_type, validator_list in pairs(self.validators) do
        for _, validator in ipairs(validator_list) do
            if validator.cleanup then
                ...
                if not success then
                    print(
                        "BalatroMCP: Warning - Failed to cleanup validator for "
                            .. action_type
                            .. ": "
                            .. tostring(error_msg)
                    )
