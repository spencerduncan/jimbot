...
local IActionValidator =
    assert(SMODS.load_file("action_executor/validators/i_action_validator.lua"))()
local ValidationResult =
    assert(SMODS.load_file("action_executor/validators/validation_result.lua"))()
...
function RerollValidator:validate(action_data, game_state)
    ...
    if not self:can_afford_reroll(game_state, reroll_cost) then
        ...
        return ValidationResult.error(
            "Insufficient funds for boss reroll ($"
                .. reroll_cost
                .. " required, $"
                .. available_funds
                .. " available)"
    ...
    if vouchers.directors_cut and not vouchers.retcon then
        ...
        if rerolls_this_ante >= 1 then
            return ValidationResult.error(
                "Director's Cut allows only 1 boss reroll per ante (used: "
                    .. rerolls_this_ante
                    .. " in ante "
                    .. current_ante
                    .. ")"
        ...
        print(
            "BalatroMCP: RerollValidator - Director's Cut validation passed for ante "
                .. current_ante
                .. " (used: "
                .. rerolls_this_ante
                .. "/1)"
    ...
    elseif vouchers.retcon then
        ...
        local rerolls_this_ante = self.reroll_tracker:get_reroll_count(current_ante)
        print(
            "BalatroMCP: RerollValidator - Retcon validation passed for ante "
                .. current_ante
                .. " (used: "
                .. rerolls_this_ante
                .. ", unlimited allowed)"
    ...
    return ValidationResult.success(
        "Boss reroll validation passed - voucher and cost requirements met"
    )
end
...
function RerollValidator:get_reroll_vouchers(game_state)
    ...
    for _, voucher in ipairs(owned_vouchers) do
        ...
        if
            voucher
            and type(voucher) == "table"
            and voucher.name
            and type(voucher.name) == "string"
        then
            local voucher_name = string.lower(voucher.name)

            -- Check for Director's Cut voucher using constants (case-insensitive)
            if
                voucher_name == VOUCHER_NAMES.DIRECTORS_CUT
                or voucher_name == VOUCHER_NAMES.DIRECTORS_CUT_ALT1
                or voucher_name == VOUCHER_NAMES.DIRECTORS_CUT_ALT2
            then
