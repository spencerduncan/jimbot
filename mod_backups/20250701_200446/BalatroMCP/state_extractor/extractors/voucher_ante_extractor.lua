...
function VoucherAnteExtractor:get_ante_requirements()
    ...
    ante_requirements.blinds_remaining =
        StateExtractorUtils.safe_get_nested_value(G, { "GAME", "round_resets", "blind" }, 0)
...
function VoucherAnteExtractor:extract_owned_vouchers()
    ...
    if
        _successful_voucher_path
        and StateExtractorUtils.safe_check_path(G, _successful_voucher_path)
    then
        local voucher_data =
            StateExtractorUtils.safe_get_nested_value(G, _successful_voucher_path, {})
        ...
        if type(voucher_data) == "table" and next(voucher_data) then
            ...
            for voucher_name, voucher_info in pairs(voucher_data) do
                if type(voucher_info) == "table" then
                    local safe_voucher = {
                        name = StateExtractorUtils.safe_primitive_value(
                            voucher_info,
                            "name",
                            voucher_name
                        ),
                        effect = StateExtractorUtils.safe_primitive_value(
                            voucher_info,
                            "effect",
                            ""
                        ),
                        description = StateExtractorUtils.safe_primitive_value(
                            voucher_info,
                            "description",
                            ""
                        ),
                        active = StateExtractorUtils.safe_primitive_value(
                            voucher_info,
                            "active",
                            true
                        ),
    ...
    for i, voucher in ipairs(G.shop_vouchers.cards) do
        if voucher and voucher.ability then
            local safe_voucher = {
                ...
                effect = StateExtractorUtils.safe_primitive_nested_value(
                    voucher,
                    { "ability", "effect" },
                    ""
                ),
                description = StateExtractorUtils.safe_primitive_nested_value(
                    voucher,
                    { "ability", "description" },
                    ""
                ),
    ...
    if StateExtractorUtils.safe_check_path(G, { "consumeables", "cards" }) then
        for i, consumable in ipairs(G.consumeables.cards) do
            if consumable and consumable.ability then
                local consumable_name = StateExtractorUtils.safe_primitive_nested_value(
                    consumable,
                    { "ability", "name" },
                    ""
                )
                local consumable_set = StateExtractorUtils.safe_primitive_nested_value(
                    consumable,
                    { "ability", "set" },
                    ""
                )
...
function VoucherAnteExtractor:is_skip_voucher_effect(name, effect)
    ...
    if not name and not effect then
        return false
    end
