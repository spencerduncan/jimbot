...
function ActionExecutor.new(state_extractor, joker_manager)
    ...
    print(
        "BalatroMCP: ActionExecutor created, validation framework will be initialized on first use"
    )
    return self
end
...
function ActionExecutor:execute_go_to_shop(action_data)
    ...
    if G.STATE ~= G.STATES.ROUND_EVAL then
        ...
        return false,
            "Cannot cash out, must be in round eval state. Current state: " .. current_state_name
    end
...
function ActionExecutor:execute_buy_item(action_data)
    ...
    if G.STATE ~= G.STATES.SHOP then
        local current_state_name = self:get_current_state_name()
        return false,
            "Cannot buy item, must be in shop state. Current state: " .. current_state_name
    end
    ...
    local shop_collections = {
        ...
        {
            collection = G.shop_consumables,
            name = "consumables",
            type = "main",
            category = "consumable",
        }, -- For planets, tarots, spectrals
        ...
        if shop_collection.collection and shop_collection.collection.cards then
            for _, card in ipairs(shop_collection.collection.cards) do
                table.insert(
                    ...
                    {
                        card = card,
                        type = shop_collection.type,
                        name = shop_collection.name,
                        category = shop_collection.category,
                    }
                )
            end
        end
    ...
    if shop_index >= #shop_items then
        return false,
            "Shop item not found at index: " .. shop_index .. " (max: " .. (#shop_items - 1) .. ")"
    end
    ...
    local is_consumable = card.ability.set == "Planet"
        or card.ability.set == "Tarot"
        or card.ability.set == "Spectral"

    print(
        "BalatroMCP: Attempting to buy "
            .. item_category
            .. " item at index "
            .. shop_index
            .. " (type: "
    ...
    if cost > available_money and cost > 0 then
        return false,
            "Not enough money: need " .. cost .. " but have " .. available_money .. " available"
    end
    ...
    if item_type == "main" then
        ...
        if action_data.buy_and_use == "true" and is_consumable then
            ...
            print(
                "BalatroMCP: Calling G.FUNCS.buy_from_shop with buy_and_use for " .. item_category
            )
...
function ActionExecutor:execute_reorder_jokers(action_data)
    ...
    if from_index >= joker_count then
        return false,
            "From index out of bounds: " .. from_index .. " (max: " .. (joker_count - 1) .. ")"
    end

    if to_index >= joker_count then
        return false,
            "To index out of bounds: " .. to_index .. " (max: " .. (joker_count - 1) .. ")"
    end
...
function ActionExecutor:execute_select_blind(action_data)
    ...
    if not blind_option then
        ...
        return false,
            "Blind option '" .. blind_key .. "' not found. Available: " .. table.concat(
                available_blinds,
                ", "
            )
    end
...
function ActionExecutor:execute_reroll_boss(action_data)
    ...
    local current_ante =
        StateExtractorUtils.safe_get_nested_value(G, { "GAME", "round_resets", "ante" }, 1)
    ...
    if G.FUNCS and G.FUNCS.reroll_boss then
        ...
        if success then
            print(
                "BalatroMCP: Boss reroll successful (ante "
                    .. current_ante
                    .. ", cost handled by game)"
            )
            return true, nil
        else
...
function ActionExecutor:execute_move_playing_card(action_data)
    ...
    if from_index >= hand_size then
        return false,
            "From index out of bounds: " .. from_index .. " (max: " .. (hand_size - 1) .. ")"
    end
...
function ActionExecutor:execute_skip_blind(action_data)
    ...
    if G.STATE ~= G.STATES.BLIND_SELECT then
        local current_state_name = self:get_current_state_name()
        return false,
            "Cannot skip blind, must be in blind selection state. Current state: "
                .. current_state_name
    end
    ...
    if not blind_option then
        ...
        return false,
            "Blind option '" .. blind_choice .. "' not found. Available: " .. table.concat(
                available_blinds,
                ", "
            )
    end
...
function ActionExecutor:execute_use_pack_tarot(action_data)
    ...
    print(
        "BalatroMCP: Using pack tarot at index: "
            .. pack_index
            .. " with "
            .. #target_card_indices
            .. " targets"
    ...
    if not pack_card.ability or pack_card.ability.set ~= "Tarot" then
        return false,
            "Pack item at index " .. pack_index .. " is not a tarot card (found: " .. tostring(
                pack_card.ability and pack_card.ability.set or "unknown"
            ) .. ")"
    end
    ...
    if #target_card_indices > 0 then
        ...
        for _, target_index in ipairs(target_card_indices) do
            ...
            if target_card then
                ...
                print(
                    "BalatroMCP: Added target card at index " .. target_index .. " to highlighted"
                )
            end
        end
    end
    ...
    print(
        "BalatroMCP: Calling G.FUNCS.use_card for tarot with "
            .. #target_card_indices
            .. " highlighted cards"
    )
    local call_success, error_result = pcall(function()
