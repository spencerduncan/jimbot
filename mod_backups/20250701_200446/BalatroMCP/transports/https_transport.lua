...
function HttpsTransport:initialize_libraries()
    ...
    self:log(
        "HTTPS transport initialized with base URL: " .. self.base_url .. " (using SMODS.https)"
    )
end
...
function HttpsTransport:make_request(method, url, body, headers)
    ...
    local success, status_code, response_body, response_headers =
        pcall(self.http_client.request, url, options)

    local end_time = os.clock()
    local duration = end_time - start_time

    print(
        "[DEBUG_STALE_STATE] SMODS.https.request result - success: "
            .. tostring(success)
            .. ", status: "
...
function HttpsTransport:is_available()
    ...
    if
        self.cached_availability ~= nil
        and (current_time - self.cached_availability_time) < self.availability_cache_duration
    then
    ...
    self:log(
        "Availability check completed in "
            .. string.format("%.3f", end_time - start_time)
            .. "s: "
            .. tostring(available)
    )
...
function HttpsTransport:write_message(message_data, message_type)
    ...
    print(
        "[DEBUG_STALE_STATE] HTTP POST result - response: "
            .. tostring(response ~= nil)
            .. ", status: "
            .. tostring(status_code)
    )
