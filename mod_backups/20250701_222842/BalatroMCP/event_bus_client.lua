...
function EventBusClient:test_connection()
    ...
    -- Check different ways SMODS might expose https
    local https_func = nil
    if SMODS and SMODS.https then
        https_func = SMODS.https
    elseif SMODS and SMODS.HTTP and SMODS.HTTP.request then
        https_func = SMODS.HTTP.request
    elseif SMODS and SMODS.HTTPS then
        https_func = SMODS.HTTPS
    end

    if not https_func then
        self.logger:error("SMODS HTTP functionality not available - cannot connect to event bus")
        ...
        if SMODS then
            ...
            self.logger:debug("SMODS contents", { keys = table.concat(smods_keys, ", ") })
        else
            self.logger:debug("SMODS is nil")
        end
...
function EventBusClient:http_post(url, data)
    -- Use stored https function
    if not self.https_func then
        self.logger:error("HTTP functionality not available")
        return false, "HTTP functionality not available"
    end

    self.logger:debug("HTTP POST via SMODS", { url = url, size = #data })
    ...
    local success, response = pcall(function()
        return self.https_func(request)
    end)
